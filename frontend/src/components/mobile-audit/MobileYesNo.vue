...
<template>
  <div class="bg-white rounded-xl border border-gray-200 px-4 py-3">
    ...
    <p v-if="description" class="pwa-body mt-0.5">{{ description }}</p>
    <div class="flex gap-2 mt-2.5">
      <button
        type="button"
        @click="$emit('update:modelValue', true)"
        :class="['pwa-button flex-1 border',
                 state === true ? 'pwa-button--on' : 'pwa-button--idle']"
      >{{ yesLabel }}</button>
      <button
        v-if="allowPartial"
        type="button"
        ...
        :class="['pwa-button flex-1 border',
                 state === null ? 'pwa-button--partial' : 'pwa-button--idle']"
      >{{ partialLabel }}</button>
      <button
        type="button"
        @click="$emit('update:modelValue', false)"
        :class="['pwa-button flex-1 border',
                 state === false ? 'pwa-button--off' : 'pwa-button--idle']"
      >{{ noLabel }}</button>
    </div>
  </div>
</template>
