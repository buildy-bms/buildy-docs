...
async function routes(fastify) {
  ...
  // ═══════════ ATELIER DESIGN — fixture audit GTB CLASSIQUE (kind=site_audit) ═══════════
  // Mêmes 4 routes parallèles que pour BACS, mais charge le fixture en
  // mode `kind: 'site_audit'`. L'audit classique partage la trame métier
  // et le design system mais sans R175 dashboard / annexes BACS.

  fastify.get('/bacs-audit/__preview-fixture-classique', async (request, reply) => {
    if (!DEV_PREVIEW_ENABLED) return reply.code(404).send({ detail: 'Not found' });
    const data = buildFixturePreviewData({ user: null, kind: 'site_audit' });
    const html = renderHtml({
      template: 'bacs-audit', styles: 'styles-bacs-audit', data, fresh: true,
    });
    const banner = buildDevBanner({
      fixtureName: 'Atlas Sud — Classique (préparation devis)',
      ts: new Date().toLocaleString('fr-FR'),
    });
    const withBanner = html.includes('</body>')
      ? html.replace('</body>', banner + '</body>')
  ...
  fastify.get('/bacs-audit/__preview-fixture-classique/pdf', async (request, reply) => {
    if (!DEV_PREVIEW_ENABLED) return reply.code(404).send({ detail: 'Not found' });
    const data = buildFixturePreviewData({ user: null, kind: 'site_audit' });
    const { version } = data;
    const exportsDir = path.resolve(config.exportsDir);
    const ts = new Date().toISOString().replace(/[:.]/g, '-');
    const filename = `fixture-classique-${version}-${ts}.pdf`;
    const fixturesOut = path.join(exportsDir, '__fixture');
    fs.mkdirSync(fixturesOut, { recursive: true });
    const outputPath = path.join(fixturesOut, filename);
    const logoSmall = loadAssetDataUrl('logo-buildy.svg');
    ...
    try {
      result = await renderPdf({
        ...
        pdfOptions: buildHeaderFooter({
          ...
          version,
          logoDataUrl: logoSmall,
          footerNote: 'Audit GTB classique · préparation devis · document fictif',
        }),
      });
    } catch (err) {
      log.error(`PDF fixture classique render failed: ${err.message}`);
      return reply.code(500).send({ detail: `Echec génération PDF fixture classique : ${err.message}` });
    }
    const pdfBuf = fs.readFileSync(result.path);
    return reply
      .header('Content-Type', 'application/pdf')
  ...
  fastify.get('/bacs-audit/__preview-fixture-classique/tables', async (request, reply) => {
    if (!DEV_PREVIEW_ENABLED) return reply.code(404).send({ detail: 'Not found' });
    const data = buildFixturePreviewData({ user: null, kind: 'site_audit' });
    const html = renderHtml({
      template: 'bacs-audit-tables', styles: 'styles-bacs-audit-tables', data,
    ...
    const banner = buildDevBanner({
      fixtureName: 'Atlas Sud — Classique tableaux (paysage)',
      ts: new Date().toLocaleString('fr-FR'),
    });
    const withBanner = html.includes('</body>')
      ? html.replace('</body>', banner + '</body>')
  ...
  fastify.get('/bacs-audit/__preview-fixture-classique/tables/pdf', async (request, reply) => {
    if (!DEV_PREVIEW_ENABLED) return reply.code(404).send({ detail: 'Not found' });
    const data = buildFixturePreviewData({ user: null, kind: 'site_audit' });
    const exportsDir = path.resolve(config.exportsDir);
    const ts = new Date().toISOString().replace(/[:.]/g, '-');
    const filename = `fixture-classique-tables-${data.version}-${ts}.pdf`;
    const fixturesOut = path.join(exportsDir, '__fixture');
    fs.mkdirSync(fixturesOut, { recursive: true });
    const outputPath = path.join(fixturesOut, filename);
    const logoSmall = loadAssetDataUrl('logo-buildy.svg');
    ...
    try {
      result = await renderPdf({
        ...
        pdfOptions: buildHeaderFooter({
          clientName: data.document.client_name,
          projectName: data.document.project_name,
          docType: 'Audit GTB — Tableaux de synthèse',
          version: data.version,
          logoDataUrl: logoSmall,
          footerNote: 'Audit GTB classique · tableaux de synthèse · document fictif',
        }),
      });
    } catch (err) {
      log.error(`PDF tables fixture classique render failed: ${err.message}`);
      return reply.code(500).send({ detail: `Echec génération PDF tables classique : ${err.message}` });
    }
    const pdfBuf = fs.readFileSync(result.path);
    return reply
      .header('Content-Type', 'application/pdf')
