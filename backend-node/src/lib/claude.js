...
async function assistFaqRewriteTitle({ article }) {
  ...
  const systemText = [
    ...
    `Critères :`,
    `- 40 à 60 caractères.`,
    `- Mot-clé métier prioritaire SEO en tête quand c'est naturel : audit BACS, logiciel GTB,`,
    `  supervision GTB, gestion technique bâtiment multi-sites, décret BACS, bâtiment tertiaire,`,
    `  parc immobilier — uniquement ceux pertinents pour le sujet.`,
    `- Évite "hypervision", "BACnet", "Modbus", "R175" qui ont peu de volume de recherche Google.`,
    `- Forme question si l'article répond à une question ("Comment réaliser un audit BACS ?"),`,
    `  sinon nominale courte ("Décret BACS : obligations pour les bâtiments tertiaires").`,
