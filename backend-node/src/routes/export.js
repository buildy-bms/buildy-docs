...
async function routes(fastify) {
  ...
  // GET /api/afs/:afId/exports/points-list/preview — apercu HTML in-browser
  // Renvoie le HTML autonome (CSS embed + fonts data URL) qu'un iframe
  // sandboxe peut afficher cote frontend. Pas de generation PDF, pas
  // d'enregistrement dans `exports`.
  fastify.get('/afs/:afId/exports/points-list/preview', async (request, reply) => {
    const afId = parseInt(request.params.afId, 10);
  ...
  // POST /api/afs/:afId/exports/points-list — generer la liste de points PDF A4 paysage
  fastify.post('/afs/:afId/exports/points-list', async (request, reply) => {
    ...
    const { data, version, categories, totals } = buildPointsListExportData(af, {
      user,
      motif: body.motif,
  ...
  // ═══════════════════════════════════════════════════════════════════
  // GET /api/afs/:afId/exports/af/preview — apercu HTML in-browser de l'AF
  // ═══════════════════════════════════════════════════════════════════
  fastify.get('/afs/:afId/exports/af/preview', async (request, reply) => {
    const afId = parseInt(request.params.afId, 10);
    const af = db.afs.getById(afId);
    if (!af || af.deleted_at) return reply.code(404).send({ detail: 'AF non trouvée' });
    const userId = request.authUser?.id;
  ...
  fastify.post('/afs/:afId/exports/af', async (request, reply) => {
    ...
    const { data, version, allSectionsCount, serviceLevel } = await buildAfExportData(af, {
      user,
      motif: body.motif,
      excludedSectionIds: body.excluded_section_ids || [],
    ...
    `).run(
      afId, result.path,
      JSON.stringify({ sections_total: allSectionsCount, service_level: serviceLevel.level }),
      JSON.stringify({ version }),
      body.motif, version, userId || null, result.sizeBytes
    );
    ...
    db.auditLog.add({
      ...
      payload: { version, motif: body.motif, sections: allSectionsCount, service_level: serviceLevel.level },
    });
    ...
    return {
      ...
      file_size_bytes: result.sizeBytes,
      download_url: `/api/exports/${insertedRow.lastInsertRowid}/download`,
      sections_total: allSectionsCount,
