 ...
 *
 * Usage côté backend :
 *   const { scoreArticle, KEYWORDS } = require('./seo-scorer');
...
// ── Whitelist des mots-clés métier Buildy ────────────────────────────
// Cette liste est partagée avec le prompt FAQ_BASE (cf. claude.js).
// Pour ajouter un mot-clé : ajouter ici ET dans le prompt SEO de claude.js.
const KEYWORDS = [
...
function scoreArticle({ title = '', contentHtml = '', targetKeywords = [] } = {}) {
  ...
  const allKeywords = [...new Set([...targetKeywords, ...KEYWORDS])];
  ...
  checks.push({
    ...
    message: foundInTitle.length > 0
      ...
      : `Aucun mot-clé métier dans le titre. Ajoute par ex. ${KEYWORDS.slice(0, 5).join(', ')}…`,
  });
