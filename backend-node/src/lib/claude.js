...
const SYSTEM_PROMPT_FAQ_BASE = [
  ...
  `- audit BACS, logiciel GTB, supervision GTB, gestion technique bâtiment multi-sites, intégrateur GTB`,
  ``,
...
const SYSTEM_PROMPT_FAQ_REWRITE = [
  ...
  ``,
  `=== TITRE (SEO) ===`,
  `Si le titre actuel est déjà clair ET contient un mot-clé pertinent (action utilisateur ou terme métier), RECOPIE-LE. Sinon propose un meilleur titre via le marker \`<!--TITLE: nouveau titre-->\` AVANT le HTML.`,
  `Critères d'un bon titre Helpdesk SEO : 40-60 caractères, commence par le mot-clé principal, formule en question si l'article répond à une question ("Comment...?"), sinon forme nominale courte. Pas de guillemets, pas de virgule, pas de point final.`,
].join('\n');
...
const SYSTEM_PROMPT_FAQ_GENERATE = [
  ...
  `Critères d'un bon titre Helpdesk SEO : 40-60 caractères, commence par le mot-clé principal (audit BACS, logiciel GTB, supervision GTB, gestion technique bâtiment multi-sites, décret BACS, etc.), formule en question si l'article répond à une question ("Comment réaliser un audit BACS de son parc immobilier ?"), sinon forme nominale courte ("Décret BACS : obligations pour les bâtiments tertiaires"). Pas de guillemets, pas de virgule, pas de point final.`,
  ``,
  ...
  `Inclure 1-2 mots-clés métier prioritaires SEO Buildy (audit BACS, logiciel GTB, supervision GTB,`,
  `gestion technique bâtiment multi-sites, décret BACS, bâtiment tertiaire, performance énergétique,`,
  `parc immobilier, property manager / asset manager — selon le sujet). Verbe d'action en tête quand`,
  `possible (Pilotez, Supervisez, Configurez, Diagnostiquez, Auditez). Pas de "Buildy" en tête, pas`,
  `de "Cet article". Évite "hypervision", "BACnet", "Modbus", "R175" qui ont un faible volume de recherche.`,
].join('\n');
...
async function assistFaqRewriteTitle({ article }) {
  ...
  const systemText = [
    ...
    `multi-sites). Tu reformules UNIQUEMENT le titre d'un article FAQ pour optimiser le`,
    `référencement Google ET attirer les bonnes personae (asset manager, property manager,`,
    `gestionnaire de parc immobilier, foncière).`,
    ``,
    `Critères :`,
    `- 40 à 60 caractères.`,
    `- MOT-CLÉ ADAPTÉ AU SUJET RÉEL DE L'ARTICLE. Ne mets PAS "supervision GTB" par défaut :`,
    `  c'est seulement le bon mot-clé si l'article parle vraiment de visualiser/piloter`,
    `  les équipements. Choisis le terme qui matche l'intention de recherche Google :`,
    `    • Audit / conformité réglementaire        → "audit BACS", "décret BACS", "mise en conformité"`,
    `    • Logiciel / produit / SaaS / offre       → "logiciel GTB"`,
    `    • Multi-sites / parc immobilier           → "gestion technique bâtiment multi-sites", "parc immobilier"`,
    `    • Pilotage temps réel / alarmes / dashboards → "supervision GTB"`,
    `    • Économies d'énergie / dérives           → "performance énergétique", "économies d'énergie"`,
    `    • App mobile / notifications              → "supervision GTB sur mobile" ou angle Gojee`,
    `    • Compatibilité avec autres normes / BREEAM/HQE → terme spécifique mentionné`,
    `    • Sujet métier transverse                 → "exploitation", "maintenance énergétique", "bâtiment tertiaire"`,
    `  Si plusieurs termes matchent, choisis celui qui décrit le mieux l'intention de`,
    `  recherche, PAS le plus générique.`,
    `- Évite "hypervision", "BACnet", "Modbus", "R175", "BAT-TH-116" qui ont peu de volume`,
    `  de recherche Google grand public.`,
    `- FORME QUESTION quand le contenu RÉPOND à une vraie question utilisateur`,
    `  ("Comment réaliser un audit BACS ?", "Le décret BACS s'applique-t-il à mon bâtiment ?").`,
    `  Sinon, forme NOMINALE courte ("Décret BACS : obligations pour les bâtiments tertiaires").`,
    `  Pas de question forcée si elle sonne creuse.`,
...
async function assistFaqRewriteSelection({ selectionHtml, instruction = '', article = null }) {
  ...
  const systemText = [
    ...
    `- Mots-clés métier SEO prioritaires (audit BACS, logiciel GTB, supervision GTB, gestion`,
    `  technique bâtiment multi-sites, décret BACS, bâtiment tertiaire, parc immobilier, asset/property manager)`,
    `  : utilise-les si naturels pour le sujet, sinon ne force pas. Évite "hypervision", "BACnet", "R175".`,
...
async function assistFaqRewriteDescription({ article }) {
  ...
  const systemText = [
    ...
    `- Inclure 1 ou 2 mots-clés métier SEO Buildy prioritaires : audit BACS, logiciel GTB,`,
    `  supervision GTB, gestion technique bâtiment multi-sites, décret BACS, bâtiment tertiaire,`,
    `  parc immobilier, performance énergétique. Selon le sujet, aussi : property/asset manager,`,
    `  pilotage à distance, alertes énergétiques, chauffage/climatisation/ventilation.`,
    ...
    `  Découvrez, Comprenez, Auditez) ou par le terme métier principal.`,
