<script setup>
/**
 * Composant CRUD pour les textes standards (annexes B & D) du PDF audit
 * BACS, table `pdf_boilerplate`. Extrait de BoilerplateAdminView.vue afin
 * d'être réutilisé en tant qu'onglet de la page « Paramètres BACS ».
 *
 * Source de vérité : table pdf_boilerplate. Lecture par kind, ordre par
 * `position`. Le PDF lit les valeurs courantes à chaque génération.
 */
  ...
  <div v-else class="space-y-8">
    ...
    <section class="bg-white rounded-lg border border-gray-200 overflow-hidden">
      <header class="px-5 py-3 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
        ...
        <button @click="addItem('methodology')" class="btn-primary text-xs px-2.5 py-1 whitespace-nowrap">
          <PlusIcon class="w-3.5 h-3.5 shrink-0" /> Ajouter un point
        </button>
      </header>
    ...
    <section class="bg-white rounded-lg border border-gray-200 overflow-hidden">
      <header class="px-5 py-3 border-b border-gray-200 bg-gray-50 flex items-center justify-between">
        ...
        <button @click="addItem('disclaimer')" class="btn-primary text-xs px-2.5 py-1 whitespace-nowrap">
          <PlusIcon class="w-3.5 h-3.5 shrink-0" /> Ajouter une mention
        </button>
      </header>
      <div class="divide-y divide-gray-100">
