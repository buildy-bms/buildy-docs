...
async function routes(fastify) {
  ...
  // ─── Preview HTML Executive Brief (synthese direction 2 pages) ─────
  fastify.get('/bacs-audit/:documentId/preview-executive', async (request, reply) => {
    const documentId = parseInt(request.params.documentId, 10);
    const af = assertBacsAuditExists(documentId, reply);
    if (!af) return;
    ...
    const html = renderHtml({
      template: 'bacs-audit-executive',
      styles: 'styles-bacs-audit-executive',
      data,
    });
    return reply.header('Content-Type', 'text/html; charset=utf-8').send(html);
  });

  // ─── Export PDF Executive Brief (synthese direction 2 pages) ───────
  // PDF court (2 pages) destine aux decideurs (DG, asset manager, proprio).
  // Reutilise les memes donnees que le rapport long, avec un template
  // dedie qui ne montre que :
  //   - verdict de conformite hero
  //   - 4 KPIs gigantesques (bloquantes / majeures / mineures / total)
  //   - applicabilite R175-2
  //   - synthese executive
  //   - top 5 actions prioritaires
  //   - donut + radar charts
  //   - contact commercial
  fastify.post('/bacs-audit/:documentId/export-pdf-executive', async (request, reply) => {
    const documentId = parseInt(request.params.documentId, 10);
    const af = assertBacsAuditExists(documentId, reply);
    if (!af) return;
    ...
    // Override version pour le tracker en bacs-exec
    const previousCount = db.db.prepare(
      `SELECT COUNT(*) AS c FROM exports WHERE af_id = ? AND kind = 'pdf-bacs-executive'`
    ).get(documentId).c;
    data.version = `bacs-exec-v${previousCount + 1}`;

    const exportsDir = path.resolve(config.exportsDir);
    const ts = new Date().toISOString().replace(/[:.]/g, '-');
    const filename = `${af.slug}-bacs-executive-${data.version}-${ts}.pdf`;
    ...
    try {
      result = await renderPdf({
        ...
        // Pas de header/footer Puppeteer : le template gere tout en interne
        // (en-tete dans la page 1, footer dans la page 2). Plus propre pour
        // un doc court de 2 pages que de repeter une bande sur chaque page.
        pdfOptions: {
          format: 'A4',
      ...
      log.error(`PDF audit BACS executive render failed: ${err.message}`);
    ...
    const insertedRow = db.db.prepare(`
      ...
      VALUES (?, 'pdf-bacs-executive', ?, ?, ?, ?, ?, ?)
    `).run(
      documentId, result.path,
      JSON.stringify({
        actions_blocking: data.actionStats.blocking,
        actions_major: data.actionStats.major,
        actions_minor: data.actionStats.minor,
      }),
    ...
    db.auditLog.add({
      afId: documentId, userId, action: 'export.bacs-audit-executive',
      payload: { version: data.version, file_size_bytes: result.sizeBytes },
    });
    log.info(`PDF audit BACS executive exported: doc #${documentId} → ${filename} (${(result.sizeBytes/1024).toFixed(1)} KB) by user #${userId}`);
